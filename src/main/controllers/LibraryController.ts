...
function getNeteaseSongIdFromPath(filePath: unknown): string | null {
    ...
    const directMatch = normalized.match(/^netease:\/+(\d+)$/);
    if (directMatch) return directMatch[1];

    const embeddedMatch = normalized.match(/(?:^|\/)netease:\/+(\d+)$/);
    return embeddedMatch ? embeddedMatch[1] : null;
}

function isNeteaseVirtualPath(filePath: unknown): boolean {
    return getNeteaseSongIdFromPath(filePath) !== null;
}

function normalizeNeteasePath(filePath: string): string {
    const songId = getNeteaseSongIdFromPath(filePath);
    return songId ? `netease://${songId}` : filePath;
}
...
export class LibraryController extends BaseController {
    ...
    @IpcHandle('library:deleteTrackFile')
    async deleteTrackFile(trackFileId: string): Promise<{ success: boolean; error?: string }> {
        try {
            const allTracks = this.libraryCacheManager.getAllTracks();
            const track = allTracks.find((t: any) =>
                t.fileId === trackFileId || t.id === trackFileId || t.filePath === trackFileId
            );
            if (!track) {
                return {success: false, error: '\u6b4c\u66f2\u4e0d\u5728\u97f3\u4e50\u5e93\u4e2d'};
            }

            const filePath = (track as any).filePath;
            if (!filePath || typeof filePath !== 'string') {
                return {success: false, error: '\u6b4c\u66f2\u6ca1\u6709\u5173\u8054\u7684\u672c\u5730\u6587\u4ef6\u8def\u5f84'};
            }

            if (isNeteaseVirtualPath(filePath)) {
                return {success: false, error: '\u7f51\u6613\u4e91\u6b4c\u66f2\u4e0d\u652f\u6301\u4ece\u78c1\u76d8\u5220\u9664'};
            }

            if ((track as any).source === 'netease' || (track as any).isVirtual === true) {
                return {success: false, error: '\u7f51\u6613\u4e91\u6b4c\u66f2\u4e0d\u652f\u6301\u4ece\u78c1\u76d8\u5220\u9664'};
            }

            if (this.networkFileAdapter.isNetworkPath(filePath)) {
                return {success: false, error: '\u7f51\u7edc\u6587\u4ef6\u6682\u4e0d\u652f\u6301\u4ece\u78c1\u76d8\u5220\u9664'};
            }

            try {
                await fs.promises.access(filePath, fs.constants.F_OK);
            } catch {
                return {success: false, error: '\u672c\u5730\u6587\u4ef6\u4e0d\u5b58\u5728\uff0c\u672a\u4fee\u6539\u97f3\u4e50\u5e93\u8bb0\u5f55'};
            }

            await fs.promises.unlink(filePath);

            this.libraryCacheManager.removeTrack(track.fileId);
            await this.libraryCacheManager.saveCache();
            console.log('[library] Deleted local file from disk:', track.title || filePath);
            return {success: true};
        } catch (error: any) {
    ...
    @IpcHandle('library:getPlaylistByExternalId')
    async getPlaylistByExternalId(externalId: string, source: string): Promise<any> {
        const playlist = this.libraryCacheManager.getPlaylistByExternalId(externalId, source);
        if (playlist) {
            return {success: true, playlist};
        }
    ...
    @IpcHandle('library:updatePlaylistMetadata')
    async updatePlaylistMetadata(playlistId: string, metadata: Record<string, unknown>): Promise<{ success: boolean; error?: string }> {
        try {
            const result = this.libraryCacheManager.updatePlaylistMetadata(playlistId, metadata);
            if (!result) return {success: false, error: '歌单不存在'};
            await this.libraryCacheManager.saveCache();
            return {success: true};
        } catch (error: any) {
            return {success: false, error: error.message};
    ...
    async getTrackMetadata(filePath: string): Promise<any> {
        try {
            ...
            if (isNeteaseVirtualPath(filePath)) {
                const normalizedPath = normalizeNeteasePath(filePath);
                const cached = this.libraryCacheManager.getTrackByPath(normalizedPath);
                if (cached) {
                    return {
                        filePath: normalizedPath,
                        title: cached.title, artist: cached.artist, album: cached.album,
                        duration: cached.duration, bitrate: cached.bitrate, sampleRate: cached.sampleRate,
                        year: cached.year, genre: cached.genre,
                        track: (cached as any).track, disc: (cached as any).disc,
                        cover: (cached as any).cover || null,
                        embeddedLyrics: (cached as any).embeddedLyrics || null,
                        source: (cached as any).source || 'netease',
                        isVirtual: true,
    ...
    async getTrackPlaybackMetadata(filePath: string): Promise<any> {
        try {
            ...
            const lookupPath = isNeteaseVirtualPath(filePath) ? normalizeNeteasePath(filePath) : filePath;
            const cachedTrack = this.libraryCacheManager.getTrackByPath(lookupPath);
            if (cachedTrack) {
                return {
                    filePath: lookupPath,
                    ...
                    cover: (cachedTrack as any).cover || null,
                    embeddedLyrics: (cachedTrack as any).embeddedLyrics || null,
                    source: (cachedTrack as any).source,
                    isVirtual: (cachedTrack as any).isVirtual,
                    libraryVisible: (cachedTrack as any).libraryVisible
                };
            }
    ...
    async addTrackToLibrary(audioFile: any): Promise<{
        ...
        try {
            ...
            let stats: any;
            const isNeteasePath = typeof audioFile.filePath === 'string' && audioFile.filePath.startsWith('netease://');
            ...
            if (isNeteasePath) {
                ...
                stats = await fs.promises.stat(audioFile.filePath);
            }
            ...
            const trackData = {
                ...
                source: isNeteasePath ? 'netease' : 'local',
                isVirtual: isNeteasePath,
                libraryVisible: !isNeteasePath
            };
            ...
            console.error('添加音频文件到音乐库失败:', error.message);
            return {success: false, error: error.message};
        }
    }
