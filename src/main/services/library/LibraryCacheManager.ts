...
export class LibraryCacheManager {
    ...
    isNetworkPath(filePath: string): boolean {
        return !!(filePath && (filePath.startsWith('network://') || filePath.startsWith('netease://')));
    }

    isOnlinePath(filePath: string): boolean {
    ...
    private async validateTrack(track: CachedTrack): Promise<ValidationResult> {
        if (this.isOnlinePath(track.filePath)) {
            return {valid: true};
        }
    ...
    addTrack(trackData: TrackData, filePath: string, stats: FileStat): CachedTrack | null {
        ...
        let hasCover = false;
        const coverValue = cover as any;
        if (typeof coverValue === 'string' && coverValue.length > 0) {
            coverUrl = coverValue;
            hasCover = true;
        } else if (coverValue && coverValue.data) {
            hasCover = true;
        }
    ...
    getPlaylistByExternalId(externalId: string, source = 'netease'): Playlist | undefined {
        return this.cache.playlists?.find(p => p.externalId === externalId && p.source === source);
    }

    updatePlaylistMetadata(playlistId: string, metadata: Partial<Pick<Playlist, 'name' | 'description' | 'source' | 'externalId' | 'externalType' | 'syncEnabled' | 'lastSyncedAt' | 'coverImagePath'>>): boolean {
        const playlist = this.cache.playlists?.find(p => p.id === playlistId);
        if (!playlist) return false;
        if (metadata.name !== undefined) playlist.name = metadata.name;
        if (metadata.description !== undefined) playlist.description = metadata.description;
        if (metadata.source !== undefined) playlist.source = metadata.source;
        if (metadata.externalId !== undefined) playlist.externalId = metadata.externalId;
        if (metadata.externalType !== undefined) playlist.externalType = metadata.externalType;
        if (metadata.syncEnabled !== undefined) playlist.syncEnabled = metadata.syncEnabled;
        if (metadata.lastSyncedAt !== undefined) playlist.lastSyncedAt = metadata.lastSyncedAt;
        const cover = metadata.coverImagePath ?? (metadata as any).coverImage;
        if (cover !== undefined) (playlist as any).coverImage = cover;
        playlist.updatedAt = Date.now();
        return true;
    }
