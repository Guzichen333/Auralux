...
import {NetEaseApiService} from '../services/netease/NetEaseApiService';
...
export class Application {
    ...
    async start(): Promise<void> {
        ...
        try {
            ...
            if (!this.isBenchmarkMode()) {
                this.startNetEaseApiService().catch(e => {
                    console.error('NetEase API startup failed:', e);
                    this.windowManager.sendToMainWindow('netease:api-unavailable', {
    ...
    private async startNetEaseApiService(): Promise<void> {
        const ready = await this.netEaseApiService.start();
        if (ready) {
            this.windowManager.sendToMainWindow('netease:api-ready', {endpoint: this.netEaseApiService.endpoint});
            return;
        }

        const recovered = await this.waitForNetEaseApiReady(120000);
    ...
    private async waitForNetEaseApiReady(timeoutMs: number): Promise<boolean> {
        ...
        while (Date.now() < deadline) {
            if (await this.netEaseApiService.isAvailable()) {
                console.log(`NetEase API became ready at ${this.netEaseApiService.endpoint}`);
                return true;
            }
    ...
    private applyConfiguration(): void {
        ...
        if (shouldDisableHardwareAcceleration) {
            ...
            if (app.isReady()) {
                ...
                if (disableHardwareAccelerationForDev) {
    ...
    private async registerStartupControllers(): Promise<void> {
        ...
        const [
            ...
            import('../controllers/ImmersiveBackgroundController'),
        ...
        const startupControllers = [
            ...
            new ImmersiveBackgroundController()
        ];
