...
export interface AudioEngineManagerBridge {
    ...
    getFrequencySpectrum?(binCount?: number): number[];
    ...
    onEngineFallback?: (from: AudioEngineType, to: AudioEngineType) => void;
}
    ...
    async initializeWebAudio(): Promise<AudioEngineManagerBridge | null> {
        try {
            ...
            if (initialized) {
                ...
                this.audioEngine.onEngineFallback = (_from, _to) => {
                    showToast('网易云歌曲已自动切换到 WebAudio 播放', 'info');
                };
