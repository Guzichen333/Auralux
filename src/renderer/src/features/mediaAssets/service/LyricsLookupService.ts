...
export class LyricsLookupService {
    ...
    async getLyrics(
        ...
        try {
            ...
            if (filePath && filePath.startsWith('netease://')) {
                const neteaseLyrics = await this.getNeteaseLyrics(filePath);
                if (neteaseLyrics.success) {
    ...
    async getNeteaseLyrics(filePath: string): Promise<LyricsResult> {
        try {
            const match = filePath.match(/^netease:\/\/(\d+)$/);
            if (!match) {
                return {success: false, error: '无效的网易云歌曲ID'};
            }

            const songId = parseInt(match[1], 10);
            const result = await netEaseLyricsService.getLyrics(songId);
            if (result && result.yrcLines.length > 0) {
                return {
                    success: true,
                    data: {lines: result.yrcLines, format: 'yrc'},
                    lyrics: {lines: result.yrcLines, format: 'yrc'},
                    content: result.yrc || undefined,
                    format: 'yrc',
                    source: 'network-lrc'
                };
            }
            ...
            if (!result || !result.lrc) {
                return {success: false, error: '网易云歌词获取失败'};
            }

            console.log(`✅ Success: 成功获取网易云歌词 (songId=${songId})`);
            return {
                success: true,
                lrc: result.lrc,
                format: 'lrc',
    ...
    private async saveLyricsToLocal(
        ...
        album: string,
        content: string,
        format: Exclude<LyricsFormat, 'yrc'>
    ): Promise<void> {
        try {
