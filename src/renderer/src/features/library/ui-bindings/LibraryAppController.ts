...
function isLibraryVisible(track: Track): boolean {
    if ((track as any).libraryVisible === false) return false;
    if ((track as any).source === 'netease') return false;
    if (typeof track.filePath === 'string' && track.filePath.startsWith('netease://')) return false;
    return true;
}

function filterLibraryTracks(tracks: Track[]): Track[] {
    return tracks.filter(isLibraryVisible);
}
    ...
    async loadInitialData(): Promise<void> {
        ...
        try {
            ...
            if (hasCachedLibrary) {
                ...
                app.library = filterLibraryTracks(await libraryService.loadCachedTracks());
            ...
            app.library = filterLibraryTracks(await libraryDataService.getTracks());
            if (app.library.length === 0) {
        ...
        try {
            app.addManagedAPIEventListener('cacheValidationCompleted', (result: MusicBoxAPIEvents['cacheValidationCompleted']) => {
                if (result.invalid > 0) {
                    ...
                    if (result.tracks) {
                        app.library = filterLibraryTracks(result.tracks);
                        app.filteredLibrary = [...app.library];
    ...
    async refreshLibrary(tracks?: Track[]): Promise<void> {
        ...
        try {
            app.library = Array.isArray(tracks)
                ? filterLibraryTracks(tracks)
                : filterLibraryTracks(await libraryDataService.getTracks());
            app.filteredLibrary = [...app.library];
            app.updateTrackList('refresh');
        } catch (error) {
            console.error('refreshLibrary 失败:', error);
        }
    }
