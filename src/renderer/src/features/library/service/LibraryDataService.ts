...
export class LibraryDataService {
    ...
    async getTrackMetadata(filePath: string): Promise<Track | null> {
        ...
        if (isNeteaseVirtualPath(filePath)) return null;
    ...
    async getPlaylistByExternalId(externalId: string, source = 'netease'): Promise<PlaylistMutationResult> {
        return await this.callGateway(
            () => libraryGateway.getPlaylistByExternalId(externalId, source),
            'library.getPlaylistByExternalId',
            {success: false, error: '查询歌单失败'}
        );
    }

    async updatePlaylistMetadata(playlistId: string, metadata: Record<string, unknown>): Promise<Result> {
        return await this.callGateway(
            () => libraryGateway.updatePlaylistMetadata(playlistId, metadata),
            'library.updatePlaylistMetadata',
            {success: false, error: '更新歌单元数据失败'}
        );
    }
    ...
    async deleteTrackFile(trackFileId: string): Promise<Result> {
        this.assertNonEmptyString(trackFileId, 'trackFileId');

        return await this.callGateway(
            () => libraryGateway.deleteTrackFile(trackFileId),
            'library.deleteTrackFile',
            {success: false, error: '从磁盘删除歌曲失败'}
        );
    }
