...
import {netEasePlaylistSyncService} from "@/features/netease/service/NetEasePlaylistSyncService";
import type {Unsubscribe} from "@api/types/common";
import type {Playlist, Track} from "@api/types/library";

function toCoverSrc(value: string | null | undefined): string {
...
function isNeteasePath(filePath: string | undefined): boolean {
    return !!filePath && filePath.startsWith('netease://');
}

function getNeteaseSongId(filePath: string): number | null {
    const match = filePath.match(/^netease:\/\/(\d+)$/);
    return match ? parseInt(match[1], 10) : null;
}

const neteaseCoverBackfillInProgress = new Set<number>();

async function backfillNeteaseCover(track: PlaylistDetailTrack): Promise<string | null> {
    if (!isNeteasePath(track.filePath)) return null;
    const songId = getNeteaseSongId(track.filePath!);
    if (!songId) return null;
    if (neteaseCoverBackfillInProgress.has(songId)) return null;

    neteaseCoverBackfillInProgress.add(songId);
    try {
        const data = await netEaseApiClient.get('/song/detail', {ids: songId.toString()});
        if (data && data.songs && data.songs[0] && data.songs[0].al && data.songs[0].al.picUrl) {
            const coverUrl = data.songs[0].al.picUrl as string;
            track.cover = coverUrl;
            try {
                await libraryController.updateTrackMetadata({
        ...
        neteaseCoverBackfillInProgress.delete(songId);
    }
}
...
class PlaylistDetailPage extends Component {
    ...
    private coverLoadInProgress: Set<string> = new Set();
    ...
    async show(playlist: PlaylistDetail): Promise<void> {
        ...
        this.coverLoadInProgress.clear();
            ...
            <div class="page-content playlist-page">
                    ...
                    <div class="hero-content">
                        ...
                        <div class="playlist-info">
                            <div class="playlist-type">
                                ...
                                <span>歌单</span>
                                ${(this.currentPlaylist as any).source === 'netease' ? '<span class="source-badge netease">网易云</span>' : ''}
                            </div>
                ...
                <div class="playlist-actions">
                    ...
                    <div class="actions-secondary">
                        ${(this.currentPlaylist as any).source === 'netease' && (this.currentPlaylist as any).externalId ? `
                        <button class="action-btn sync-netease" id="playlist-sync-netease">
                            <svg class="icon" viewBox="0 0 24 24">
                                <path d="M12,4V1A11,11 0 0,0 1,12H3A9,9 0 0,1 12,3V6L16,2L12,0V4M21,12A9,9 0 0,1 12,21V22A11,11 0 0,0 23,12H21M12,18A6,6 0 0,0 6,12H2A10,10 0 0,1 12,22V18M12,8A4,4 0 0,1 16,12H20A8,8 0 0,0 12,4V8Z"/>
                            </svg>
                            <span>刷新</span>
                        </button>
                        ` : ''}
    ...
    private async handleActionButtonClick(button: HTMLElement): Promise<void> {
        ...
        switch (button.id) {
            ...
            case 'playlist-clear':
                ...
                break;
            case 'playlist-sync-netease':
                await this.syncNetEasePlaylist();
                break;
            default:
    ...
    async syncNetEasePlaylist(): Promise<void> {
        if (!this.currentPlaylist) return;
        const playlist = this.currentPlaylist as any;
        if (playlist.source !== 'netease' || !playlist.externalId) return;

        const syncBtn = document.getElementById('playlist-sync-netease') as HTMLButtonElement | null;
        if (syncBtn) {
        ...
        try {
            const result = await netEasePlaylistSyncService.syncPlaylist(playlist.id, playlist.externalId);

            if (result.success) {
                let msg = `同步完成: 新增 ${result.addedCount} 首`;
                if (result.removedRemoteCount > 0) {
                    msg += `，云端已移除 ${result.removedRemoteCount} 首（本地暂保留）`;
                }
                showToast(msg, 'success');

                const detail = await playlistDataService.getPlaylistDetail(playlist.id);
                if (detail && detail.success) {
                    this.currentPlaylist = detail.playlist || this.currentPlaylist;
            ...
            if (syncBtn) {
                ...
                syncBtn.querySelector('span')!.textContent = '刷新';
            }
        }
    }
    ...
    getTrackCover(track: PlaylistDetailTrack): string {
        if (track.cover && typeof track.cover === 'string') {
            return toCoverSrc(track.cover);
        }

        if (isNeteasePath(track.filePath)) {
            void this.backfillNeteaseCoverAndUpdate(track);
    ...
    private async backfillNeteaseCoverAndUpdate(track: PlaylistDetailTrack): Promise<void> {
        const coverUrl = await backfillNeteaseCover(track);
        if (!coverUrl) return;
        ...
        this.requestAnimationFrameManaged(() => {
            ...
            trackRows.forEach((row) => {
                ...
                const idx = parseInt(trackRow.dataset.trackIndex || '-1', 10);
                if (idx >= 0 && this.tracks[idx] === track) {
                    const coverImg = trackRow.querySelector<HTMLImageElement>('.track-cover');
                    if (coverImg) {
                        coverImg.src = toCoverSrc(coverUrl);
                    }
                }
            });
            this.updatePlaylistHeaderCover();
        });
    }

    private updatePlaylistHeaderCover(): void {
        if (!this.container) return;
        const hasManualCover = this.currentPlaylist && this.currentPlaylist.coverImage;
        if (hasManualCover) return;

        const firstCover = this.tracks.find(t => t.cover && typeof t.cover === 'string');
        if (!firstCover) return;

        const coverEl = this.container.querySelector('#playlist-cover');
        if (!coverEl) return;
        const existingImg = coverEl.querySelector('img.cover-image');
        if (existingImg) return;

        coverEl.innerHTML = `<img class="cover-image" src="${toCoverSrc(firstCover.cover as string)}" alt="歌单封面" />`;
    }

    async loadTrackCoverAsync(track: PlaylistDetailTrack): Promise<void> {
        const trackKey = track.filePath || `${track.title}_${track.artist}`;

        if (this.coverLoadInProgress.has(trackKey)) {
            return;
        }
        ...
        this.coverLoadInProgress.add(trackKey);
            ...
            const loadCover = async () => {
                ...
                if (coverResult.success && coverResult.imageUrl && typeof coverResult.imageUrl === 'string') {
                    ...
                    if (coverResult.type === 'local-file' && coverResult.filePath) {
                        coverUrl = toCoverSrc(coverResult.filePath);
                    }
                    ...
                    this.requestAnimationFrameManaged(() => {
                        ...
                        trackRows.forEach((row, index) => {
                            ...
                            if (parseInt(trackRow.dataset.trackIndex || '0') === index && this.tracks[index] === track) {
                                const coverImg = trackRow.querySelector<HTMLImageElement>('.track-cover');
                                if (coverImg) {
                                    const newSrc = toCoverSrc(coverUrl);
                    ...
                    console.warn(`PlaylistDetailPage: 封面加载失败 - ${track.title}:`, coverResult.error || '未知错误');
                }
            };
    ...
    renderPlaylistCover(): string {
        if (this.currentPlaylist && this.currentPlaylist.coverImage) {
            return `<img class="cover-image" src="${toCoverSrc(this.currentPlaylist.coverImage)}" alt="歌单封面" />`;
        }

        const firstCoverTrack = this.tracks.find(t => t.cover && typeof t.cover === 'string');
        if (firstCoverTrack) {
            return `<img class="cover-image" src="${toCoverSrc(firstCoverTrack.cover as string)}" alt="歌单封面" />`;
        }
