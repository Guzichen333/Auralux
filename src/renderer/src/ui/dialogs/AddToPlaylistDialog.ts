...
class AddToPlaylistDialog extends Component {
    ...
    async addToPlaylist(playlistId: string): Promise<void> {
        if (!this.currentTrack) {
            this.emit('notification', {type: 'error', message: '当前歌曲无效'});
            return;
        }

        try {
            let trackToAdd = this.currentTrack;

            // 网易云曲目需要先入库获取 fileId
            if (!trackToAdd.fileId && this.isNeteaseTrack(trackToAdd)) {
                const addResult = await libraryController.addTrackToLibrary({
                ...
                if (!addResult.success || !addResult.track?.fileId) {
                    this.emit('notification', {type: 'error', message: '添加到音乐库失败'});
                    return;
                }

                trackToAdd = {...trackToAdd, fileId: addResult.track.fileId} as any;
            }
            ...
            const result = await playlistDialogActionService.addTrackToPlaylist(playlistId, trackToAdd, this.playlists);
            if (result.success) {
                const playlist = result.playlist;
                this.emit('notification', {type: 'info', message: `已添加到歌单 "${playlist?.name || '未知'}"`});
                this.emit('trackAdded', {playlist, track: trackToAdd});
                this.hide();
            } else {
    ...
    private isNeteaseTrack(track: any): boolean {
        if (track.source === 'netease') return true;
        if (typeof track.filePath === 'string' && track.filePath.startsWith('netease://')) return true;
        return false;
    }
