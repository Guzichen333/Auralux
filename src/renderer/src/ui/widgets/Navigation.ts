...
class Navigation extends Component {
    ...
    renderPlaylistItem(playlist: SidebarPlaylist): string {
        const coverSrc = this.toCoverSrc(playlist.coverImage);
        if (this.sidebarCollapsed) {
            return `
                <li>
                    <div class="playlist-sidebar-item collapsed-item" data-playlist-id="${playlist.id}" title="${this.escapeHtml(playlist.name)} (${playlist.trackIds ? playlist.trackIds.length : 0} 首歌曲)">
                        ${coverSrc ? `
                            <img class="sidebar-playlist-cover" src="${coverSrc}" alt="歌单封面" />
                        ` : `
    ...
    private toCoverSrc(coverImage: string | undefined | null): string | null {
        if (!coverImage) return null;
        if (coverImage.startsWith('http://') || coverImage.startsWith('https://') ||
            coverImage.startsWith('file://') || coverImage.startsWith('data:') ||
