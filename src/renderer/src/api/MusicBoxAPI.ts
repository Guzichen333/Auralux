...
export class MusicBoxAPI extends EventEmitter {
    ...
    private activeLoadFilePath: string | null = null;
    ...
    setupEventListeners(): void {
        ...
        if (audioEngine) {
            audioEngine.onTrackChanged = async (track: unknown) => {
                const trackFilePath = (track as any)?.filePath || (track as any)?.path || null;
                if (this.activeLoadFilePath && trackFilePath && trackFilePath !== this.activeLoadFilePath) {
                    console.log(`⏭️ API: 忽略旧 trackChanged (期望=${this.activeLoadFilePath}, 收到=${trackFilePath})`);
                    return;
                }
                ...
                this.playbackPersistence.saveQueueSnapshot();
    ...
    async loadTrack(filePath: string): Promise<boolean> {
        ...
        this.activeLoadFilePath = filePath;
        ...
        try {
            ...
            if (this.audioEngine) {
                ...
                if (seq !== this.loadRequestSeq) {
                    console.log(`⏭️ API: loadTrack [${seq}] 被新请求覆盖，跳过后续同步`);
                    return false;
                }
                if (result) {
            ...
            if (seq !== this.loadRequestSeq) return false;
            if (result) {
    ...
    setPendingTrack(track: Track): void {
    ...
    getFrequencySpectrum(binCount = 64): number[] {
        try {
            return this.audioEngine?.getFrequencySpectrum?.(binCount) || [];
        } catch (error) {
            console.error('Failed to get frequency spectrum:', error);
            return [];
        }
    }
    ...
    getLastLoadError(): string | null {
        return this.audioEngine?.lastLoadError || null;
    }
