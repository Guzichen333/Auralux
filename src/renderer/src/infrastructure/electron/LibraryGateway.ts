...
class LibraryGateway extends ElectronNamespaceAdapter<'library'> {
    ...
    getPlaylistByExternalId(externalId: string, source: string): Promise<PlaylistResult> {
        return this.call('getPlaylistByExternalId', externalId, source);
    }

    updatePlaylistMetadata(playlistId: string, metadata: Record<string, unknown>): Promise<Result> {
        return this.call('updatePlaylistMetadata', playlistId, metadata);
    }
    ...
    deleteTrackFile(trackFileId: string): Promise<Result> {
        return this.call('deleteTrackFile', trackFileId);
    }
